...
// MARK: - View of List of Favorites
class FavoritesViewController: UIViewController, FavoritesPresenterDelegate {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(self.setFavoritesMovies),
                                               name: NSNotification.Name(rawValue: "CallFavoritesUpdate"),
                                               object: nil)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == Segues.segueFilter {
            ...
            if let yourNextViewController = (segue.destination as? FilterViewController) {
...
extension FavoritesViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       if  let cell = tableView.dequeueReusableCell(withIdentifier: Cells.movieFavoriteCell, for: indexPath)
            as? FavoriteViewCell {
